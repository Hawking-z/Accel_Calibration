...
uint8_t MPU6050_init(MPU6050_imu* obj) {
    ...
    if (res == obj->config.addr) {
        ...
        MPU_Set_Rate(obj, 200);                        //设置采样率为200Hz
        return 0;
    } else {
        bsp_delay_ms(10);
        return 1;
...
void MPU6050_Update(MPU6050_imu* obj) {
    ...
    obj->calibration_data.gyro[0] = obj->data.gyro[0];
    obj->calibration_data.gyro[1] = obj->data.gyro[1];
    obj->calibration_data.gyro[2] = obj->data.gyro[2];
    obj->data.accel[0] = obj->data.accel[0]*obj->config.accelfactor[0]+obj->config.accelbias[0];
    obj->data.accel[1] = obj->data.accel[1]*obj->config.accelfactor[1]+obj->config.accelbias[1];
    obj->data.accel[2] = obj->data.accel[2]*obj->config.accelfactor[2]+obj->config.accelbias[2];
